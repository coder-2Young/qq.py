...
parser.add_argument("--data_path",type=str, default="datasets/train",help='path to training data')
...
def main():
    ...
    # Build model
    model = network(recurrent_iter=opt.recurrent_iter, use_GPU=opt.use_gpu)
    print_network(model)
    ...
    for epoch in range(initial_epoch, opt.epochs):
        ...
        for i, (input_train, target_train) in enumerate(loader_train, 0):
            ...
            psnr_train = batch_PSNR(out_train, target_train, 1.)  # 记录本batch的PSNR
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.data_path.find('Rain12600') != -1:
