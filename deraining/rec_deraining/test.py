...
parser.add_argument("--logdir", type=str, default="logs/network/", help='path to model and log files')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = network(opt.recurrent_iter, opt.use_GPU)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
